...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        if ($update->execute()) {
            $message = "<div class='bg-green-100 text-green-700 p-3 rounded-md mb-4'>✅ Password updated successfully!</div>";
        } else {
...
<!-- HEADER -->
<section class="bg-gray-800 text-white text-center py-10 shadow-md">
  <h1 class="text-3xl font-bold">Change Password</h1>
  <p class="opacity-90">Manage your account security</p>
</section>
...
<section class="py-10 bg-gray-50 px-4 min-h-screen">
  <div class="max-w-xl mx-auto bg-white shadow-lg rounded-xl p-8">
    ...
    <form method="POST" class="space-y-5">
      <div>
        ...
        <input type="password" name="current_password"
               class="w-full border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-gray-700 focus:outline-none" required>
      </div>
      ...
      <div class="text-center">
        <button type="submit" class="bg-gray-800 text-white px-6 py-3 rounded-md font-semibold hover:bg-gray-900 transition">
          Update Password
        </button>
      </div>
    </form>
    ...
    <div class="text-center mt-6">
      <a href="dashboard.php" class="text-gray-700 hover:underline">
        ← Back to Dashboard
      </a>
    </div>
  </div>
